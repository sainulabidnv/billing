...
switch ($act) {
    case "create_quote" :
        ...
        if ($crepeat == "yes" and $bill_csd == "0") {
            ...
            if ($bill_csd == "0") {
                ...
                $dbg = $db->insert('damage_customers', $dataArray);
            }
        ...
        $status = $db->insert('damage', $dataArray)->rStatus();
        foreach ($_POST['bill_saman'] as $key => $value) {
            ...
            $db->insert('damage_items', $dataArray);

            //If you like to add stock relations with damages    
            /*
            $data = $item_product;
        ...
        if ($status == true) {
            ...
            echo json_encode(array('status' => 'Success', 'message' =>
                "Damage Receipt has been created successfully! &nbsp; &nbsp;<a href='view/damage-view.php?id=$bill_number' class='btn btn-warning btn-lg'><span class='icon-print' aria-hidden='true'></span> Print </a> &nbsp; <a href='view/damage-view.php?id=$bill_number&download=1' class='btn btn-info btn-lg'><span class='icon-download2' aria-hidden='true'></span> Download </a>"));
    ...
    case "edit_quote" :
        if ($user->group_id <= 2) {
            ...
            $aWhere = array('tid' => $id);
            $status = $db->update('damage', $dataArray, $aWhere)->rStatus();
            ...
            if ($cidd == 0) {
                ...
                $aWhere = array('tid' => $id);
                $db->delete('damage_customers', $aWhere);


                $dataArray = array('tid' => $bill_number, 'name' => $grahak_name, 'address1' => $grahak_adrs1, 'address2' => $grahak_adrs2, 'phone' => $grahak_phone, 'email' => $grahak_email, 'taxid' => $grahak_tax);
                $isnr = $db->insert('damage_customers', $dataArray)->rStatus();
            }

            $aWhere = array('tid' => $id);
            $db->delete('damage_items', $aWhere);
            foreach ($_POST['bill_saman'] as $key => $value) {
                $item_product = $value;
            ...
            if ($status == true) {
                ...
                echo json_encode(array('status' => 'Success', 'message' =>
                    "Damage Receipt has been updated successfully! &nbsp; &nbsp;<a href='view/damage-view.php?id=$bill_number' class='btn btn-warning btn-lg'><span class='icon-print' aria-hidden='true'></span> Print </a> &nbsp; <a href='view/damage-view.php?id=$bill_number&download=1' class='btn btn-info btn-lg'><span class='icon-download2' aria-hidden='true'></span> Download </a>"));
    ...
    case "delete_quote" :
        if ($user->group_id <= 2) {
            ...
            $aWhere = array('tid' => $id);
            $limit = 'LIMIT 1';
            $status = $db->delete('damage', $aWhere, $limit)->affectedRows();
            $aWhere = array('tid' => $id);
            $db->delete('damage_items', $aWhere);
            $aWhere = array('tid' => $id);
            $db->delete('damage_customers', $aWhere, $limit);


            if ($status == 1) {
                echo json_encode(array('status' => 'Success', 'message' =>
                    'Damage Receipt has been deleted successfully!'));
