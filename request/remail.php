...
$whereConditions = array('tid' => $id);
if ($typ == 'invoice') {
    $colm = array('total', 'ramm');
    $rresult = $db->select('invoices', $colm, $whereConditions)->results();
    $totala = $rresult['total'];
    $remain = $rresult['ramm'];
    $famt = $totala - $remain;
    $invoice = 'invoice';
} else {
    $colm = array('total');
    $rresult = $db->select('rec_invoices', $colm, $whereConditions)->results();
    $totala = $rresult['total'];
//$remain=$rresult['ramm'];
    $famt = $totala;
    $invoice = 'rinvoice';
...
$mail->Subject = "Payment Reminder Invoice $id";
//Read an HTML message body from an external file, convert referenced images to embedded,
//convert HTML into a basic plain-text alternative body
$mail->msgHTML("<div><strong>Dear $cname,</strong><br>
<p>This is a notification regarding payment of Invoice no $id.</p>
<h3>Total Due Amount is " . $siteConfig['curr'] . "$famt</h3>
