<?php if (stristr(htmlentities($_SERVER['PHP_SELF']), "checkout-rec.php")) {
...
$row = $db->select('rec_invoices', array('tid', 'total', 'tsn_due', 'csd', 'rperiod', 'rc_next', 'rc_up'), array('tid' => $getID))->results();
...
switch ($rperiod) {
    case 1:
        $rc_next = date("Y-m-d", strtotime("+7 days", strtotime($next_pm)));
        break;
    case 2:
        $rc_next = date("Y-m-d", strtotime("+15 days", strtotime($next_pm)));
        break;
    case 3:
        $rc_next = date("Y-m-d", strtotime("+30 days", strtotime($next_pm)));
        break;
    case 4:
        $rc_next = date("Y-m-d", strtotime("+3 months", strtotime($next_pm)));
        break;
    case 5:
        $rc_next = date("Y-m-d", strtotime("+6 months", strtotime($next_pm)));
        break;
    case 6:
        $rc_next = date("Y-m-d", strtotime("+1 years", strtotime($next_pm)));
        break;
    case 7:
        $rc_next = date("Y-m-d", strtotime("+3 years", strtotime($next_pm)));
        break;
}
...
if (isset($_GET['token']) == $validtoken) {
    ...
    //			<a href='view/invoice-view.php?id=$bill_number' class='btn btn-warning btn-lg'><span class='icon-print' aria-hidden='true'></span> Print </a> &nbsp; <a href='view/invoice-view.php?id=$bill_number&download=1' class='btn btn-info btn-lg'><span class='icon-download2' aria-hidden='true'></span> Download </a></div></div>";
...
if ($_POST) {
    ...
    try {
        ...
        $query = "UPDATE rec_invoices SET rc_next = '$rc_next', rc_up = '$tod',ramm = ramm + $total WHERE tid='$bill_number'";
        $aBindWhereParam = array('tid' => $bill_number);
        $db->pdoQuery($query, $aBindWhereParam)->results();

        $dataArray = array('tid' => $bill_number, 'amount' => $total, 'tdate' => $tod);
                    ...
                    <div class="col-md-6 panel panel-default  mp">
                        ...
                        <form action="https://www.paypal.com/cgi-bin/webscr" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-7">
                                    <strong><?php echo $inext_pm ?><br>&nbsp;</strong></div>
