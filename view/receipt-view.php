...
$query = "SELECT rterms,footer FROM billing_terms";
$row_conf = $db->select('billing_terms')->results();
$terms = $row_conf['rterms'];
$footer = $row_conf['footer'];
require_once("../includes/system.php");
require_once("../includes/globalcf.php");
$query = "SELECT p.*, i.*
			FROM receipts_items p 
			JOIN receipts i ON i.tid = p.tid			
			WHERE p.tid = '$getID'";
...
foreach ($result as $row) {
    ...
    if ($cidd > 0) {
        ...
        $whr = array('id' => $cidd);
        $row1 = $db->select('reg_vendors', null, $whr)->results();
        ...
        $whr = array('tid' => $getID);
        $row1 = $db->select('vendors', null, $whr)->results();
        $grahak_name = $row1['name'];
        $grahak_email = $row1['email'];
        $grahak_adrs1 = $row1['address1'];
        $grahak_adrs2 = $row1['address2'];
