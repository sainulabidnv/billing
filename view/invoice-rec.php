...
$query = "SELECT terms,footer FROM billing_terms";
$row_conf = $db->select('billing_terms')->results();
$terms = $row_conf['terms'];
$footer = $row_conf['footer'];
require_once("../includes/system.php");
require_once("../includes/globalcf.php");
$query = "SELECT p.*, i.*
			FROM rec_items p 
			JOIN rec_invoices i ON i.tid = p.tid			
			WHERE p.tid = '$getID' AND i.active=0";
...
if ($result = $db->pdoQuery($query)->results()) {
    foreach ($result as $row) {
        ...
        $next_date = dateformat($row['rc_next']);
        $last_date = $row['rc_up'];
        switch ($row['rperiod']) {
            case 1:
                $rc_next = "7 days";
        ...
        if ($cidd > 0) {
            ...
            $whr = array('tid' => $getID);
            $row1 = $db->select('rec_customers', null, $whr)->results();
            $grahak_name = $row1['name'];
            $grahak_email = $row1['email'];
            $grahak_adrs1 = $row1['address1'];
            $grahak_adrs2 = $row1['address2'];
    ...
    $grahak_postcode = "";
    include('../includes/express-rec.php');
...
//Set due date
    ...
    $pdf->nextDate($next_date);
    if ($last_date != '0000-00-00') {
        $pdf->lastDate(dateformat($last_date));
    }
    $pdf->recP('Recurrence on ' . $rc_next);
    $pdf->currF($siteConfig['curr']);
//Biller address
    $pdf->biller($siteConfig['name'], $siteConfig['address'], $siteConfig['address2'],
        "Phone: " . $siteConfig['phone'], "Mail: " . $siteConfig['email'], "VAT/TAX No " .
        $siteConfig['vatno']);
//Customer address
