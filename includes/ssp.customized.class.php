...
class SSP
{
        ...
        for ($i = 0, $ien = count($data); $i < $ien; $i++) {
            ...
            for ($j = 0, $jen = count($columns); $j < $jen; $j++) {
                ...
                if (isset($column['formatter'])) {
                    $row[$column['dt']] = ($isJoin) ? $column['formatter']($data[$i][$column['field']],
                        $data[$i]) : $column['formatter']($data[$i][$column['db']], $data[$i]);
                } else {
        ...
        if (isset($request['order']) && count($request['order'])) {
            ...
            for ($i = 0, $ien = count($request['order']); $i < $ien; $i++) {
                ...
                if ($requestColumn['orderable'] == 'true') {
                    ...
                    $orderBy[] = ($isJoin) ? $column['db'] . ' ' . $dir : '`' . $column['db'] . '` ' .
                        $dir;
                }
            }
    ...
    static function filter($request, $columns, &$bindings, $isJoin = false)
    {
        ...
        if (isset($request['search']) && $request['search']['value'] != '') {
            ...
            for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
                ...
                if ($requestColumn['searchable'] == 'true') {
                    ...
                    $globalSearch[] = ($isJoin) ? $column['db'] . " LIKE " . $binding : "`" . $column['db'] .
                        "` LIKE " . $binding;
                }
            }
        }
        ...
        for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
            ...
            if ($requestColumn['searchable'] == 'true' && $str != '') {
                $binding = SSP::bind($bindings, '%' . $str . '%', PDO::PARAM_STR);
                $columnSearch[] = ($isJoin) ? $column['db'] . " LIKE " . $binding : "`" . $column['db'] .
                    "` LIKE " . $binding;
            }
        }
        ...
        if (count($columnSearch)) {
            $where = $where === '' ? implode(' AND ', $columnSearch) : $where . ' AND ' .
     ...
     *
     */
    static function simple($request, $sql_details, $table, $primaryKey, $columns, $joinQuery = null,
                           $extraWhere = '', $groupBy = '')
    {
        $bindings = array();
        ...
        if ($joinQuery) {
            ...
            $query = "SELECT SQL_CALC_FOUND_ROWS `" . implode("`, `", SSP::pluck($columns,
                    'db')) . "`
			 FROM `$table`
			 $where
			 $extraWhere
    ...
    static function isimple($request, $sql_details, $table, $primaryKey, $columns, $joinQuery = null,
                            $extraWhere = '', $groupBy = '')
    {
        $bindings = array();
        $db = SSP::sql_connect($sql_details);
    ...
    static function sql_connect($sql_details)
    {
        try {
            ...
            SSP::fatal("An error occurred while connecting to the database. " .
    ...
    */
    ...
    static function pluck($a, $prop, $isJoin = false)
    {
        ...
        for ($i = 0, $len = count($a); $i < $len; $i++) {
            $out[] = ($isJoin && isset($a[$i]['as'])) ? $a[$i][$prop] . ' AS ' . $a[$i]['as'] :
                $a[$i][$prop];
        }
